...
<<<<<<< HEAD
    ...
    if n_samples < 2:
        raise RuntimeError(
            f"Za mało sesji do klastrowania (n_samples={n_samples}). "
            "Zbierz więcej danych (uruchom clean na pełnym zbiorze) albo obniż --min-commands."
        )
...
<<<<<<< HEAD
    ...
    n_components = 2 if min(n_for_coords, n_features) >= 2 else 1
